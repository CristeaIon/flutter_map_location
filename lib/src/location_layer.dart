...
import 'package:geolocator/geolocator.dart' show Geolocator, LocationServiceDisabledException, LocationSettings;
...
class _LocationLayerState extends State<LocationLayer> with WidgetsBindingObserver {
  final ValueNotifier<LocationServiceStatus> _serviceStatus = ValueNotifier<LocationServiceStatus>(LocationServiceStatus.unknown);
  ...
  void initState() {
    super.initState();
    _controller = widget.options.controller as LocationControllerImpl? ?? LocationController() as LocationControllerImpl;
      ...
      children: <Widget>[
        ValueListenableBuilder<LatLngData?>(
            ...
            builder: (BuildContext context, LatLngData? ld, Widget? child) {
              ...
              final Marker marker = customBuilder != null ? customBuilder(context, ld, _heading) : _defaultMarkerBuilder(context, ld, _heading);
  ...
  void _initOnLocationUpdateSubscription() async {
    ...
    _locationSub = _controller.subscribePosition(widget.options.locationSettings ?? const LocationSettings()).listen((LatLngData loc) {
      _location.value = loc;
